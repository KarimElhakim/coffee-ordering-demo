...
export const createOrder = useRealSupabase
  ? async (orderData: {
      ...
      // Calculate discount amount if not provided but percentage is
      ...
      // Calculate tax if not provided (default 14% VAT for Egypt)
      ...
      // Insert order items with all details including product snapshots
      const orderItems = orderData.items.map((item) => {
        ...
        return {
          ...
          note: item.note || null,
          // Financial breakdown (calculated by trigger, but set here for clarity)
          subtotal: itemSubtotal,
          discount_amount: 0, // Line item discounts can be added later
          tax_amount: itemSubtotal * 0.14, // 14% VAT per line item
      ...
      // Insert order item options (modifiers) with all details
      ...
      // Return order with all details including order_number
      return order;
    }
  : demoApi.createOrder;
...
export const updateOrderStatus = useRealSupabase
  ? async (orderId: string, status: 'new' | 'paid' | 'in_prep' | 'ready' | 'served' | 'cancelled') => {
      const { data, error } = await supabase
        .from('orders')
        .update({ status })
        .eq('id', orderId)
